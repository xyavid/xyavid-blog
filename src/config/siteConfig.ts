 ...
 * 部署参数（唯一需要按环境改的地方）
 *
 * 当前对应仓库 xyavid/xyavid-blog，线上地址 https://xyavid.github.io/xyavid-blog/
 * GitHub Actions 部署时不用手改：工作流会注入 SITE_URL / BASE_PATH 两个环境变量。
 * 想在本地按根路径预览：BASE_PATH=/ pnpm dev
 */
const SITE_URL = process.env.SITE_URL ?? "https://xyavid.github.io";
const BASE_PATH = process.env.BASE_PATH ?? "/xyavid-blog";
// site_url 只填 origin（不含子路径），子路径由 astro.config.mjs 的 base 处理
const SITE_ORIGIN = SITE_URL.replace(/\/+$/, "");

// 页面开关配置 - 控制特定页面的访问权限，设为 false 会返回 404 并自动隐藏对应的导航栏菜单项
// 本项目只保留原有站点用到的页面，Firefly 的社交/相册/追番等模块一律关闭
...
export const siteConfig: SiteConfig = {
	...
	// 站点关键词
	keywords: ["xyavid", "博客", "技术", "随笔", "项目", "Astro"],
	...
	// 导航栏配置
	navbar: {
		// 导航栏Logo：使用站点自带的 SVG 图标，随亮暗主题自动切换颜色
		logo: {
			type: "icon",
	...
	post: {
		...
		// 是否显示随机文章推荐
		randomPosts: false,
		// OpenGraph图片功能。开启后每篇文章都要在构建时渲染一张 PNG，
		// 这是全站最慢的一步，内容还没多到需要它，先关掉。
